...
- **`dynamodb:ListTables` granted account-wide.** Same shape as DescribeLogGroups — the action doesn't accept resource-tag conditions. Devs see table names across the account; actual table content remains bounded by the tag-scoped DynamoDB statement.
- **`codebuild:ListProjects` and `scheduler:ListSchedules` / `ListScheduleGroups` granted account-wide.** Same shape as the others above — List actions don't accept resource scope. Devs see project / schedule names across the account; details (`BatchGetProjects`, `GetSchedule`) remain tag- or name-prefix-bound.
